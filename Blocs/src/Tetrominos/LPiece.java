	...
	public boolean isClearBelow(TetrisBoard board) {
		...
		switch (getRotationBy90()) {
			...
			case 1:
				...
				for (int i = 0; i < 3; i++) {
					if (i == 0) {
						b3 = board.isBooleanTileFilled(x+(i*TetrisBoard.TILE_DIMEN), y+TetrisBoard.TILE_DIMEN*2);
					} else {
			...
			case 2:
				//x, y is 2nd down
				if (board.isBooleanTileFilled(x, y+TetrisBoard.TILE_DIMEN*2) || board.isBooleanTileFilled(x-TetrisBoard.TILE_DIMEN, y)) {
					Log.d("case 0 is NOT clear below", "x is "+x+" y is "+y);
					return false;
				} else {
					return true;
